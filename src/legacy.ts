...
const start = () => {
  const config = configFromSearch(window.location.search);
  const url = new URL(window.location.href);
  url.searchParams.set("seed", String(config.seed));
  window.history.replaceState(null, "", url);
  return new Game(canvas, config);
};
...
if (header) {
  new ResizeObserver(() => {
    document.documentElement.style.setProperty("--observer-top", `${header.getBoundingClientRect().bottom + 8}px`);
  }).observe(header);
}
