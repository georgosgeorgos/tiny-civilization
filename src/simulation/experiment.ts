...
export function runExperiment(plan: ExperimentPlan): ExperimentRecord {
  const years = Math.max(0, Math.floor(finiteDuration(plan.years, "years")));
  const interval = Math.max(1, Math.floor(finiteDuration(plan.checkpointEvery ?? 10, "checkpointEvery")));
  ...
  const checkpoints: ExperimentCheckpoint[] = [{ year: 0, snapshot: engine.checkpoint }];
  let elapsed = 0;
  while (elapsed < years) {
...
export function branchExperiment(plan: BranchPlan): ExperimentCheckpoint[] {
  const years = Math.max(0, finiteDuration(plan.years, "years"));
  const interval = Math.max(1, Math.floor(finiteDuration(plan.checkpointEvery ?? 10, "checkpointEvery")));
  ...
  const checkpoints: ExperimentCheckpoint[] = [{ year: plan.checkpoint.year, snapshot: engine.checkpoint }];
  let elapsed = 0;
  while (elapsed < years) {
    const span = Math.min(interval, years - elapsed);
    advanceInAnnualSteps(engine, inputs, span);
...
export function runSweep(config: SweepConfig): SweepSummary[] {
  const interval = Math.max(1, Math.floor(finiteDuration(config.checkpointEvery ?? 10, "checkpointEvery")));
  const years = Math.max(0, Math.floor(finiteDuration(config.years, "years")));
